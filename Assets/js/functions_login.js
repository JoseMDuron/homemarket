...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (document.querySelector("#formLogin")) {
        //si existe agregamos el evento con una variable con var, indica que se va utilizar solo en esta funcion
        var formLogin = document.querySelector("#formLogin");
        formLogin.onsubmit = function (e) {
            e.preventDefault();//prevenir a que se recargue la pagina al momento de darle submit

            var strEmail = document.querySelector('#txtEmail').value;
            var strPassword = document.querySelector('#txtPasswordl').value;
    ...
    if (document.querySelector("#form_reiniciar_contraseñas")) {//se dirige hacia el elemento reiniciar contraseña de la vista
        var form_reiniciar_contraseñas = document.querySelector("#form_reiniciar_contraseñas");//si existe ese elemento hace la validacion y le asigna a una variable que obtiene el id del formulario
        form_reiniciar_contraseñas.onsubmit = function (e) {
            ...
            var strEmail = document.querySelector('#txt_correo_reiniciar').value;//obtiene el valor del input del correo
            var rbPreguntas = document.querySelector('#op_preguntas').value;
            var rbCorreo = document.querySelector('#op_correo').value;
    ...
    if (document.querySelector('#formCambiarPass')) {
        ...
        var formCambiarPass = document.querySelector('#formCambiarPass');//con esta variable asignamos el formulario de cambiar contrasena
        formCambiarPass.onsubmit = function (e) {
            e.preventDefault();//previene que se recargue la pagina al momento de darle aceptar al formulario
            var strPassword = document.querySelector('#txtPassword').value;//asignamos los valores que obtenemos de los inputs
            var strPasswordConfirm = document.querySelector('#txtPasswordConfirm').value;
            var idUsuario = document.querySelector('#id_Usuario').value;
    ...
    if (document.querySelector('#formCambiarPass2')) {
        var formCambiarPass = document.querySelector('#formCambiarPass2');//con esta variable asignamos el formulario de cambiar contrasena
        formCambiarPass.onsubmit = function (e) {
            e.preventDefault();//previene que se recargue la pagina al momento de darle aceptar al formulario
            var strPassword = document.querySelector('#txtPassword').value;//asignamos los valores que obtenemos de los inputs
            var strPasswordConfirm = document.querySelector('#txtPasswordConfirm').value;
            var strPasswordAnterior = document.querySelector('#txtPasswordAnterior').value;
            var idUsuario = document.querySelector('#id_Usuario').value;
    ...
    if (document.querySelector('#formPreguntas')) {
        var formPreguntas = document.querySelector('#formPreguntas');//con esta variable asignamos el formulario de cambiar contrasena
        formPreguntas.onsubmit = function (e) {
            e.preventDefault();//previene que se recargue la pagina al momento de darle aceptar al formulario
            var strRespuesta = document.querySelector('#txtRespuestaPregunta').value;//asignamos los valores que obtenemos de los inputs
    ...
    if (document.querySelector('#formPrimer_Ingreso')) {//validar las preguntas y respuestas del usuario
        var formPrimer_Ingreso = document.querySelector('#formPrimer_Ingreso');//con esta variable asignamos el formulario de cambiar contrasena
        formPrimer_Ingreso.onsubmit = function (e) {
            e.preventDefault();//previene que se recargue la pagina al momento de darle aceptar al formulario
          
            var strRespuesta = document.querySelector('#txt_Respuesta').value;
...
function checkPassword(){
    ...
    var regex = new RegExp(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&#.$($)$-$_])[A-Za-z\d$@$!%*?&#.$($)$-$_]{8,15}$/);
...
function fntPreguntas() {
    ...
    if (document.querySelector('#form_list_preguntas')) {
        ...
        var ajaxUrl = base_url + '/Login/getSelectPreguntas';
     //   var idUsuario = document.querySelector('#id_Usuario').value;
    ...
    if (document.querySelector('#formPrimer_Ingreso')) {
        var formPrimer_Ingreso = document.querySelector("#formPrimer_Ingreso");
        var ajaxUrl = base_url + '/Login/getSelectPreguntas';
     //   var idUsuario = document.querySelector('#id_Usuario').value;
        var request = (window.XMLHttpRequest) ?
            new XMLHttpRequest() :
